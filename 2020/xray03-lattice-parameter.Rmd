...
Question
========
  Examine the powder X-ray diffraction spectrum. One of the peaks is labelled with its Miller Indices, $(hkl) \; = \; `r labels` $. Use the position, $2\theta_{ `r labels`}$ of this peak to calculate the interplanar spacing and thence the lattice parameter, $a$, in nm. The value for the X-ray wavelength is $\lambda\;=\;`r lambda`nm$.
...
spectrum %>% ggplot(aes(two_theta, intensity)) + 
  ...
  geom_text_repel(data = fcc[index,], aes(x = 2*theta, y = height+0.0015, label = hkl)) +
  labs(x = "2θ") +
